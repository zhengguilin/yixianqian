...
public class F5 {
    static Pattern pattern = Pattern.compile("import (.*?);");
    ...
    public static void main(String args[]) {
        try {
            F5.readfile("D:/workspace/workspace_trunk1/branch-quartz-maven/src");
            FileWriter fw=new FileWriter("d:/test.txt");
            for (int i=0;i<F5.list.size();i++) {
                if(F5.list.get(i).getNum()==0) {
                    fw.write(F5.list.get(i).getPath()+":"+F5.list.get(i).getFile1()+":"+F5.list.get(i).getFile()+"\r\n");
                }
            }
            fw.close();
        } catch (FileNotFoundException e) {
